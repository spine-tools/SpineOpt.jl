...
@testset "generate missing items" begin
    ...
    @testset for (_class_name, name, _default_value) in template_parameters
        @test Symbol(name) in all_names
        @test getfield(SpineOpt, Symbol(name)) isa Parameter
    end
end