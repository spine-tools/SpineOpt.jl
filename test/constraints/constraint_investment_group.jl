...
function _test_equal_investments()
    @testset "equal_investments" begin
        ...
        object_parameter_values = [["investment_group", "ig", "equal_investments_active", true]]
        SpineInterface.import_data(url_in; object_parameter_values=object_parameter_values)
        m = run_spineopt(url_in; log_level=0, optimize=false)
        constraint = m.ext[:spineopt].constraints[:investment_group_equal_investments]
        unit_ab = SpineOpt.unit(:unit_ab)
        connection_bc = SpineOpt.connection(:connection_bc)
        node_c = SpineOpt.node(:node_c)
        parent = SpineOpt.stochastic_scenario(:parent)
        t4h = first(time_slice(m; temporal_block=SpineOpt.temporal_block(:investments_four_hourly)))
        key_head = (investment_group=SpineOpt.investment_group(:ig), entity1=unit_ab)
        ...
        node_c_inv_avail = [
            ...
            for t in time_slice(m; temporal_block=SpineOpt.temporal_block(:investments_two_hourly))
        ]
...
function _test_min_max_capacity_invested_available()
    @testset "min_max_capacity_invested_available" begin
        ...
        relationship_parameter_values = [
            ...
            ("connection__to_node", ("connection_bc", "node_c"), "capacity_per_connection", 250),
        ]
