    ...
    @testset "constraint_user_constraint_slack_penalty_original_coarse_resolution" begin
        ...
        @testset for sense in ("==", ">=", "<=")
            ...
            relationship_parameter_values = [
                ...
                [relationships[1]..., "storages_invested_available_coefficient", 0.0], # Required for the coarser original resolution caused by the default zeroes.
            ]
            SpineInterface.import_data(
                url_in;
                objects=objects,
    ...
    @testset "constraint_user_constraint_slack_penalty_new_higher_resolution" begin
        ...
        @testset for sense in ("==", ">=", "<=")
            ...
            for t1h in t1h_arr
                ...
                exp_con = SpineOpt.build_sense_constraint(
                    node_state_coefficient * var_n_state[node_c, parent, t1h]
