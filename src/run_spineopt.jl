...
function add_variables!(m; log_level=3)
    ...
    # TODO: @timelog log_level 3 "- [variable_ramp_costs]" add_variable_ramp_costs!(m)
    ...
    @timelog log_level 3 "- [variable_nonspin_units_starting_up]"  add_variable_nonspin_units_starting_up!(m)
...
function add_constraints!(m; add_constraints=m -> nothing, log_level=3)
    ...
    # TODO: @timelog log_level 3 "- [constraint_ramp_cost]" add_constraint_ramp_cost!(m)
...
function _save_variable_value!(m::Model, name::Symbol, indices::Function)
    var = m.ext[:variables][name]
    m.ext[:values][name] = Dict(ind => _variable_value(var[ind]) for ind in indices(m))
end
