...
function constraint_fix_ratio_out_in_flow(m::Model, flow)
    @butcher for (u, cg_out, cg_in) in unit__out_commodity_group__in_commodity_group()
        ...
        ## get all time_slices for which the flow variables are defined (direction = :out)
            for c_out in commodity_group__commodity(commodity_group=cg_out), (n,tblock) in commodity__node__unit__direction__temporal_block(unit = u,direction = :out, commodity = c_out), t in time_slice(temporal_block=tblock)
        ...
        ## get all time_slices for which the flow variables are defined (direction = :in)
            for c_in in commodity_group__commodity(commodity_group=cg_in), (n,tblock) in commodity__node__unit__direction__temporal_block(unit = u,direction = :in,commodity = c_in), t in time_slice(temporal_block=tblock)
                        time_slices_constraint_in= push!(time_slices_constraint_in,t)
            end
            ## remove duplicates (e.g. if to flows of the same direction are defined on the same temp level)
...
######## give flow keys? e.g. for flow in flowkeys ...
     for t in constraint_generate_on
        all([
