...
"""
    constraint_max_node_voltage_angle_indices(m::Model; filtering_options...)
...
"""
function constraint_max_node_voltage_angle_indices(m::Model; node=anything, stochastic_path=anything, t=anything)
    unique(
        (node=ng, stochastic_path=path, t=t)
        for (ng, s, t) in node_voltage_angle_indices(m; node=node) if ng in indices(max_voltage_angle)
