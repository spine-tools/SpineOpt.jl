...
@doc raw"""
Limit the decrease of [unit\_flow](@ref var_unit_flow) over a time period of one [duration\_unit](@ref) according
...
& + \frac{\sum_{n \in ng, \: t' \in overlapping(t)}
v^{unit\_flow}_{(u,n,d,s,t')} \cdot \Delta(t'\cap t) \cdot \left[ p^{reserve\_active}_{(n)} \land p^{reserve\_downward}_{(n)} \right]}{\Delta(overlapping(t))} \\
...
\frac{\sum_{t' \in overlapping(t-1)} \left( p^{minimum\_operating\_point}_{(u,ng,d,s,t)}
 \cdot v^{units\_on}_{(u,s,t')} \cdot \Delta(t'\cap t-1) \right) }{\Delta(overlapping(t-1))}   \\
...
function add_constraint_ramp_down!(m::Model)
    _add_constraint!(m, :ramp_down, constraint_ramp_down_indices, _build_constraint_ramp_down)
end
...
function _build_constraint_ramp_down(m::Model, u, ng, d, s_path, t_before, t_after)
    ...
    @build_constraint(
        ...
        + sum(
            ...
            if reserve_active(node=n)
            && _switch(d; to_node=reserve_downward, from_node=reserve_upward)(node=n)
