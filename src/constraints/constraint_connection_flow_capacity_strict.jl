...
"""
    add_constraint_connection_flow_capacity_strict!(m::Model)

Limit the maximum in/out `connection_flow` of a `connection` for all `connection_flow_capacity_strict` indices.
...
"""
function add_constraint_connection_flow_capacity_strict!(m::Model)
    @fetch connection_flow, connections_invested_available = m.ext[:variables]
    t0 = startref(current_window(m))
    m.ext[:constraints][:connection_flow_capacity_strict] = Dict(
        (connection=conn, node=ng, direction=d, stochastic_path=s, t=t) => @constraint(
            ...
            duration(t)            
        ) for (conn, ng, d, s, t) in constraint_connection_flow_capacity_strict_indices(m)
    )
end

"""
    constraint_connection_flow_capacity_strict_indices(m::Model; filtering_options...)

Form the stochastic index array for the `:connection_flow_capacity_strict` constraint.

Uses stochastic path indices of the `connection_flow` variables. Only the lowest resolution time slices are included,
as the `:connection_flow_capacity_strict` is used to constrain the "average power" of the `connection`
instead of "instantaneous power". Keyword arguments can be used to filter the resulting 
"""
function constraint_connection_flow_capacity_strict_indices(
    m::Model;
    connection=anything,
    node=anything,
    dir=anything,
    ...
    unique(
        ...
        for (c, ng, d) in indices(connection_capacity; connection=connection, node=node, direction=dir)            
        for d in [direction(:from_node), direction(:to_node)]
        ...
        path in active_stochastic_paths(unique(
            ind.stochastic_scenario for ind in _constraint_connection_flow_capacity_strict_indices(m, c, ng, d, t)            
        )) if path == stochastic_path || path in stochastic_path
    )
end

"""
    _constraint_connection_flow_capacity_strict_indices(connection, node, direction1, node2, direction2, t)

Gather the indices of the relevant `unit_flow` and `units_on` variables.
"""
function _constraint_connection_flow_capacity_strict_indices(m, connection, node, direction, t)
