    ...
    m.ext[:constraints][:unit_state_transition] = Dict(
        (unit=u, stochastic_path=s, t_before=t_before, t_after=t_after) => @constraint(
            ...
            expr_sum(
                ...
                for (u, s, t_after) in units_on_indices(m; unit=u, stochastic_scenario=s, t=t_after, temporal_block=anything);
                init=0,
            ) == expr_sum(
                +units_on[u, s, t_before]
                for (u, s, t_before) in units_on_indices(m; unit=u, stochastic_scenario=s, t=t_before, temporal_block=anything);
                init=0,
            )
...
function constraint_unit_state_transition_indices(
    ...
    unique(
        ...
        path in active_stochastic_paths(unique(
            ind.stochastic_scenario for ind in units_on_indices(m; unit=u, t=[t_before, t_after], temporal_block=anything)
