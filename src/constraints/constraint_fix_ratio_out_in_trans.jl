...
function constraint_fix_ratio_out_in_trans(m::Model, trans)
    @constraint(
        ...
        + reduce(
            ...
            for (c_out) in commodity__node__connection__direction__temporal_block(
                    node=node_out, connection=conn, direction=:out,temporal_block=:any
                )
        ...
        + fix_ratio_out_in_trans_t(connection=conn, node1=node_in, node2=node_out, temporal_block=tblock)
            * reduce(
                ...
                for (c_in) in commodity__node__connection__direction__temporal_block(
                        node=node_in, connection=conn, direction=:in,temporal_block=:any
                    )
                    for t2 in t_in_t(t_long=t)
