...
function add_constraint_min_node_voltage_angle!(m::Model)
    ...
    m.ext[:constraints][:min_node_voltage_angle] = Dict(
        (node=ng, stochastic_scenario=s, t=t) => @constraint(
            m,
            + expr_sum(
                + node_voltage_angle[ng, s, t] for (ng, s, t) in node_voltage_angle_indices(m; node=ng, stochastic_scenario=s, t=t);
                init=0,
            ) >=
            + min_voltage_angle[(node=ng, stochastic_scenario=s, analysis_time=t0, t=t)]
            #TODO: support investments?
        ) for (ng, s, t) in constraint_min_node_voltage_angle_indices(m)
    )
end

"""
    constraint_min_node_voltage_angle_indices(m::Model; filtering_options...)
...
"""
function constraint_min_node_voltage_angle_indices(m::Model; node=anything, stochastic_path=anything, t=anything)
    unique(
        (node=ng, stochastic_path=path, t=t)
        for (ng, s, t) in node_voltage_angle_indices(m; node=node) if ng in indices(min_voltage_angle)
