    ...
    m.ext[:constraints][:min_start_up_ramp] = Dict(
        (unit=u, node=ng, direction=d, stochastic_path=s, t=t) => @constraint(
            ...
            +sum(
                ...
                (u, s, t) in units_on_indices(m; unit=u, stochastic_scenario=s, t=t_overlaps_t(m; t=t))
            )
...
"""
    constraint_min_start_up_ramp_indices(m::Model; filtering_options...)
...
function constraint_min_start_up_ramp_indices(
    ...
    unique(
        ...
        (u, ng, d) in indices(min_startup_ramp) if u in unit && ng in node && d in direction for
