...
function constraint_ratio_out_in_connection_flow_indices(ratio_out_in)
    ...
    for (conn, ng_out, ng_in) in indices(ratio_out_in)
        for t in t_lowest_resolution(x.t for x in connection_flow_indices(connection=conn, node=[ng_out, ng_in]))
            ...
            append!(
                ...
                map(
                    ...
                    connection_flow_indices(
                        connection=conn, node=ng_out, direction=direction(:to_node), t=t_in_t(t_long=t)
                    )
                )
            )
