...
"""
    constraint_user_constraint_indices(m::Model; filtering_options...)
...
"""
function constraint_user_constraint_indices(
    m::Model;
    ...
    unique(
        (user_constraint=uc, stochastic_path=path, t=t) for uc in user_constraint
        for t in _constraint_user_constraint_lowest_resolution_t(m, uc, t)
        for
        path in active_stochastic_paths(unique(
            ind.stochastic_scenario for ind in _constraint_user_constraint_indices(m, uc, t)
...
"""
function _constraint_user_constraint_lowest_resolution_t(m, uc, t)
    t_lowest_resolution(
        vcat(
            [ind.t for unit__node__user_constraint in (unit__from_node__user_constraint, unit__to_node__user_constraint)
            for (u, n) in unit__node__user_constraint(user_constraint=uc)
            for ind in unit_flow_indices(m; unit=u, node=n, t=t)],
            [ind.t for connection__node__user_constraint in (connection__from_node__user_constraint, connection__to_node__user_constraint)
            ...
            [ind.t for n in node__user_constraint(user_constraint=uc)
...
function _constraint_user_constraint_unit_flow_indices(m, uc, t)
    (
        ind for (u, n) in unit__from_node__user_constraint(user_constraint=uc) for
...
"""
    _constraint_user_constraint_connectiojn_flow_indices(uc, t)
...
function _constraint_user_constraint_connection_flow_indices(m, uc, t)
    (
        ind for (c, n) in unit__from_node__user_constraint(user_constraint=uc) for
...
function _constraint_user_constraint_node_stochastic_time_indices(m, uc, t)
    (
        ind for n in node__user_constraint(user_constraint=uc) for
...
function _constraint_user_constraint_units_on_indices(m, uc, t)
    (
        ind for u in unit__user_constraint(user_constraint=uc) for
...
"""
function _constraint_user_constraint_indices(m, uc, t)
    Iterators.flatten((
