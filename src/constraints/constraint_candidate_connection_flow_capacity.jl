...
"""
    add_constraint_candidate_connection_flowcapacity!(m::Model)

Limit the maximum in/out `candidate_connection_flow` of a `connection` for all `candidate_connection_flow_capacity` indices.
...
handles `candidate_connection_flows` to multiple `nodes` with different temporal resolutions, the constraint is only generated
for the lowest resolution, and only the average of the higher resolution `candidate_connection_flow` is constrained.
If instantaneous power needs to be constrained as well, defining the `connection_capacity` separately for each
`candidate_connection_flow` can be used to achieve this.
"""
function add_constraint_candidate_connection_flow_capacity!(m::Model)
    @fetch candidate_connection_flow, connections_invested_available = m.ext[:variables]
    ...
    m.ext[:constraints][:candidate_connection_flow_capacity] = Dict(
        (connection=conn, node=ng, direction=d, stochastic_path=s, t=t) => @constraint(
            m,
            +expr_sum(
                candidate_connection_flow[conn, n, d, s, t] * duration(t)
                for
                (conn, n, d, s, t) in candidate_connection_flow_indices(
                    m;
                    connection=conn,
                    direction=d,
                    node=ng,
            ...
            duration(t) <=
            +expr_sum(
                candidate_connection_flow[conn, n, d_reverse, s, t] * duration(t)
                for
                (conn, n, d_reverse, s, t) in candidate_connection_flow_indices(
                    m;
                    connection=conn,
                    node=ng,
                    stochastic_scenario=s,
        ...
        ) for (conn, ng, d, s, t) in constraint_candidate_connection_flow_capacity_indices(m)
    )
end

"""
    constraint_candidate_connection_flowcapacity_indices(m::Model; filtering_options...)

Form the stochastic index array for the `:candidate_connection_flowcapacity` constraint.

Uses stochastic path indices of the `connection_flow` variables. Only the lowest resolution time slices are included,
as the `:candidate_connection_flowcapacity` is used to constrain the "average power" of the `connection`
instead of "instantaneous power". Keyword arguments can be used to filter the resulting 
"""
function constraint_candidate_connection_flow_capacity_indices(
    m::Model;
    connection=anything,
    node=anything,
    direction=anything,
    ...
    unique(
        ...
        path in active_stochastic_paths(unique(
            ind.stochastic_scenario for ind in _constraint_candidate_connection_flow_capacity_indices(m, c, ng, d, t)            
        )) if path == stochastic_path || path in stochastic_path
    )
end

"""
    _constraint_candidate_connection_flow_capacity_indices(connection, node, direction1, node2, direction2, t)

Gather the indices of the relevant `unit_flow` and `units_on` variables.
"""
function _constraint_candidate_connection_flow_capacity_indices(m, connection, node, direction, t)
    (m, connection, node, direction, t)
    Iterators.flatten((
        candidate_connection_flow_indices(m; connection=connection, node=node, direction=direction, t=t),        
