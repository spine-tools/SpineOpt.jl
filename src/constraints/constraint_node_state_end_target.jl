...
"""
    add_constraint_node_state_end_target!(m::Model)

Constrain the  value of the last `node_state` variable in the rolling window.
"""
function add_constraint_node_state_end_target!(m::Model)
    @fetch node_state, storages_invested_available = m.ext[:variables]
    t0 = startref(current_window(m))
    m.ext[:constraints][:node_state_end_target] = Dict(
        (node=ng, stochastic_scenario=s, t=t) => @constraint(
            ...
            ==
            +node_state_end_target[(node=ng, stochastic_scenario=s, analysis_time=t0, t=t)]
        ...
        ) for (ng, s, t) in constraint_node_state_end_target_indices(m)
    )
end


"""
    constraint_node_state_end_target_indices(m::Model; filtering_options...)
...
"""
function constraint_node_state_end_target_indices(
    m::Model;    
    node=anything,    
    ...
    unique(
        ...
        for (ng, s, t) in node_state_last_index(m; node=node)
        if ng in indices(node_state_end_target)
        for
