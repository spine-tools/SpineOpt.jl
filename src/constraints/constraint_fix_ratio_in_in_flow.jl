...
is specified.
"""
@catch_undef function constraint_fix_ratio_in_in_flow(m::Model)
    @fetch flow = m.ext[:variables]
    constr_dict = m.ext[:constraints][:fix_ratio_in_in_flow] = Dict()
    for (u, c1, c2) in indices(fix_ratio_in_in_flow)
        ...
        for t in t_lowest_resolution(involved_timeslices)
            constr_dict[u, c1, c2, t] = @constraint(
                ...
                ==
                + fix_ratio_in_in_flow(unit=u, commodity1=c1, commodity2=c2, t=t)
                * sum(
