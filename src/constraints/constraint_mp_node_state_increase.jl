...
"""
    add_constraint_mp_node_state_increase!(m::Model)

Limit the increase in node state between timeslices in the master problem to `decomposed_max_state_increase`, if it exists.

"""
function add_constraint_mp_node_state_increase!(m::Model)
    @fetch mp_node_state = m.ext[:variables]
    t0 = startref(current_window(m))
    m.ext[:constraints][:mp_node_state_increase] = Dict(
        (node=ng, stochastic_scenario=s, t_before=t_before, t_after=t_after) => @constraint(
            ...
            <=
            +decomposed_max_state_increase[(node=ng, stochastic_scenario=s, analysis_time=t0, t=t_before)]            
            *min(duration(t_before), duration(t_after))
        ) for (ng, s, t_before, t_after) in constraint_mp_node_state_increase_indices(m)
    )
end


"""
    constraint_mp_node_state_increase_indices(m::Model; filtering_options...)
...
"""
function constraint_mp_node_state_increase_indices(
    m::Model;    
    node=anything,    
    ...
    unique(
        ...
        for (ng, s, t_after) in mp_node_state_indices(m; node=node) 
            if ng in mp_storage_node && ng in indices(decomposed_max_state_increase)
