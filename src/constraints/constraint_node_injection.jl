...
If a node corresponds to a storage node, the parameter [storage\_active](@ref)
...
& \forall n \in node: p^{storage\_active}_{(n)}\\
& \forall (s, t)
\end{aligned}
```
...
function constraint_node_injection_indices(m::Model)
    (
        ...
        if (storage_active(node=n) && storage_longterm_active(node=n)) || _is_representative(t_after)
