    ...
    m.ext[:constraints][:node_injection] = Dict(
        (node=n, stochastic_path=s, t_before=t_before, t_after=t_after) => @constraint(
            ...
            +expr_sum(
                ...
                node_injection_indices(m; node=n, stochastic_scenario=s, t=t_after, temporal_block=anything) for
                ng in groups(n);
                init=0,
            ) ==
            +expr_sum(
            ...
            expr_sum(
                ...
                diff_coeff[(node1=other_node, node2=n, stochastic_scenario=s, analysis_time=t0, t=t_after)] for
                other_node in node__node(node2=n) for s in s;
                init=0,
            )
            ...
            expr_sum(
                get(node_state, (n, s, t_after), 0) *
                diff_coeff[(node1=n, node2=other_node, stochastic_scenario=s, analysis_time=t0, t=t_after)] for
                other_node in node__node(node1=n) for s in s;
                init=0,
            )
...
function constraint_node_injection_indices(
    ...
    unique(
        ...
        (n, t_before, t_after) in node_dynamic_time_indices(m; node=node) for path in active_stochastic_paths(
