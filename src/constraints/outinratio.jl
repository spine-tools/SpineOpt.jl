function outinratio(m::Model, flow)
    @constraint(m, [u in unit(), n in node(), t=1:number_of_timesteps("timer"); !isnull(FixRatioOutputInputFlow_ElectricityGas(u))],
    ...
    @constraint(m, [u in unit(), n in node(), t=1:number_of_timesteps("timer"); !isnull(FixRatioOutputInputFlow_ElectricityCoal(u))],
        + sum(flow["Electricity", n, u, "out", t]  for n in CommodityAffiliation("Electricity") if all([u in NodeUnitConnection(n), u in output_com("Electricity")]))
    ...
    @constraint(m, [u in unit(), n in node(), t=1:number_of_timesteps("timer"); !isnull(FixRatioOutputInputFlow_HeatGas(u))],
