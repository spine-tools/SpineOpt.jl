...
function _do_solve_multi_stage_model!(
    ...
    for i in Iterators.countfrom(1)
        ...
        if is_adaptive
            ...
            termination_msg = "Multi-stage execution complete"
        end
        if termination_msg !== nothing
            @log log_level 1 termination_msg
        ...
        empty!.(m.ext[:spineopt].extras[:total_costs_by_time_stamp] for m in all_models)
    end
    true
end
...
function _save_total_costs_by_time_stamp!(m, _k)
    ...
    merge!(get!(m.ext[:spineopt].extras, :total_costs_by_time_stamp, Dict()), total_costs_by_time_stamp)
end
...
function _update_stage_models!(m, max_gap; log_level)
    ...
    for (st, stage_m) in m.ext[:spineopt].model_by_stage
        ...
        stage_costs = sum(values(stage_m.ext[:spineopt].extras[:total_costs_by_time_stamp]))
        child_models = _child_models(m, st)
        max_child_costs = maximum(
            sum(values(child_m.ext[:spineopt].extras[:total_costs_by_time_stamp])) for child_m in child_models
        )
...
function _diff_ts(stage_m, child_m)
    stage_costs_by_t = stage_m.ext[:spineopt].extras[:total_costs_by_time_stamp]
    child_costs_by_t = child_m.ext[:spineopt].extras[:total_costs_by_time_stamp]
...
function _do_enforce_integrality!(m)
    for (name, vars) in m.ext[:spineopt].variables
        @fetch bin, int = m.ext[:spineopt].variables_definition[name]
        for (ind, var) in vars
            _set_binary(var, _resolve(bin, ind))
            _set_integer(var, _resolve(int, ind))
        end
    end
    for stage_m in values(m.ext[:spineopt].model_by_stage)
