...
function ramp_up_unit_flow_indices(
    ...
    unique(
        ...
        for
        (u, n, d, tb) in
        ramp_up_unit__node__direction__temporal_block(unit=unit, node=node, direction=direction, temporal_block=temporal_block, _compact=false)
        for
        (n, s, t) in
...
function add_variable_ramp_up_unit_flow!(m::Model)
    ...
    add_variable!(
        ...
        ramp_up_unit_flow_indices;
        lb=x -> 0,
        fix_value=x -> fix_ramp_up_unit_flow(
            unit=x.unit,
            node=x.node,
