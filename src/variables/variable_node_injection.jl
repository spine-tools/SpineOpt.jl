...
"""
function node_injection_indices(
    m::Model;
    node=anything,
    ...
    #(
    #    (node = n, stochastic_scenario = s, t = ta) 
    #    for (n, s, t1) in node_stochastic_time_indices(m)
    #    for (n, tb, ta) in node_dynamic_time_indices(m, node = n, t_after = t1)
    #)
end
