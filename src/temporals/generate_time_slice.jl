...
function generate_time_slice()
    ...
    for k in temporal_block()
        ...
        #    for x in #collect(start_date(k):Minute(time_slice_duration()[:temporal_block][k][1]):end_date(k)-Minute(time_slice_dur#ation()[:temporal_block][k][1]))
        #        time_slice_symbol = #Symbol("t_$(year(x))_$(month(x))_$(day(x))_$(hour(x))_$(minute(x))__$(year(x+Minute(time_slice_duration(#)[:temporal_block][k][1])))_$(month(x+Minute(time_slice_duration()[:temporal_block][k][1])))_$(day(x+Min#ute(time_slice_duration()[:temporal_block][k][1])))_$(hour(x+Minute(time_slice_duration()[:temporal_bloc#k][k][1])))_$(minute(x+Minute(time_slice_duration()[:temporal_block][k][1])))")
        #        list_time_slice = push!(list_time_slice,time_slice_symbol)
        #        list_timesliceblock[k] = push!(list_timesliceblock[k],time_slice_symbol)
        ...
        #        time_slice_symbol = #Symbol("t_$(year(x))_$(month(x))_$(day(x))_$(hour(x))_$(minute(x))__$(year(x+Minute(time_slice_duration(#)[:temporal_block][k][j])))_$(month(x+Minute(time_slice_duration()[:temporal_block][k][j])))_$(day(x+Min#ute(time_slice_duration()[:temporal_block][k][j])))_$(hour(x+Minute(time_slice_duration()[:temporal_bloc#k][k][j])))_$(minute(x+Minute(time_slice_duration()[:temporal_block][k][j])))")
        #        list_time_slice = push!(list_time_slice,time_slice_symbol)
        #        list_timesliceblock[k] = push!(list_timesliceblock[k],time_slice_symbol)
    ...
    function time_slice(;temporal_block=nothing) # propose to rename to time_slice
