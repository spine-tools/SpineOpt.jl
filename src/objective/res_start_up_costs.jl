...
function res_start_up_costs(m::Model, t1)
    ...
    @expression(
        ...
        expr_sum(
            ...
            unit_stochastic_scenario_weight(m; unit=u, stochastic_scenario=s) for
            (u, n, d) in indices(res_start_up_cost) for
            (u, n, s, t) in nonspin_units_started_up_indices(m; unit=u, node=n) if end_(t) <= t1;
            init=0,
        )
    )
end
