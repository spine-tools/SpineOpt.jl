...
function connection_investment_costs(m::Model, t1)
    ...
    @expression(
        ...
        +expr_sum(
            ...
            connection_stochastic_scenario_weight(m; connection=c, stochastic_scenario=s) for
            (c, s, t) in connections_invested_available_indices(m; connection=indices(connection_investment_cost)) if
            end_(t) <= t1;
            init=0,
        )
    )
