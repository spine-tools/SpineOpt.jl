    ...
    let tax_costs = zero(AffExpr)
        for (u, n, d, block) in indices(vom_cost)
            ...
            + reduce(
                ...
                for (cg1, ng1) in indices(tax_net_flow) for (u, n, c, d, t) in flow_indices(
            ...
            + reduce(
                ...
                for (cg1,ng1) in indices(tax_out_flow) for (u, n, c, d, t) in flow_indices(
                    node=node_group__node(node_group = ng1),
                    commodity=commodity_group__commodity(commodity_group=cg1),
                    direction=:out
                );
            ...
            + reduce(
                ...
                for (cg1,ng1) in indices(tax_in_flow) for (u, n, c, d, t) in flow_indices(
                    node=node_group__node(node_group = ng1),
                    commodity=commodity_group__commodity(commodity_group=cg1),
                    direction=:in
                );
