...
function taxes(m::Model, t1)
    ...
    @expression(
        ...
        +expr_sum(
            ...
            node_stochastic_scenario_weight[(node=n, stochastic_scenario=s)] for (n,) in indices(tax_net_unit_flow)
        ...
        expr_sum(
            ...
            node_stochastic_scenario_weight[(node=n, stochastic_scenario=s)] for (n,) in indices(tax_out_unit_flow)
            for (u, n, d, s, t) in unit_flow_indices(m; node=n, direction=direction(:from_node)) if end_(t) <= t1;
            init=0,
        ) +
        expr_sum(
