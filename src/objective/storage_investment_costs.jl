...
function storage_investment_costs(m::Model, t1)
    ...
    @expression(
        ...
        +expr_sum(
            ...
            node_stochastic_scenario_weight[(node=n, stochastic_scenario=s)] for
            (n, s, t) in storages_invested_available_indices(m; node=indices(storage_investment_cost)) if end_(t) <= t1;
            init=0,
        )
    )
end
