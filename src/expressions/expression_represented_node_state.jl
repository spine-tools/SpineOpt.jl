...
function add_expression_represented_node_state!(m::Model)
    @fetch node_injection, node_state, unit_flow, node_slack_pos, node_slack_neg = m.ext[:spineopt].variables
    m.ext[:spineopt].expressions[:represented_node_state] = Dict(
        (node=n, stochastic_path=s_path, t=t_after) => @expression(
            m,
        ...
        # Diffusion of commodity from this node to other nodes                             
                sum(
                    duration(t_after) / (
                        ...
                        + duration(t_after) * frac_state_loss(m; node=n, stochastic_scenario=s, t=t_after)
                    )
                    for s in s_path                    
                ) * 
                (            
                ...
                + sum(
                    (
                        + get(represented_node_state, (n, s, t_before), 0)
        ...
        for (n, s_path, t_before, t_after) in expression_represented_node_state_indices(m)
    ) 
end

function expression_represented_node_state_indices(m::Model)
    (
...
function expression_represented_node_state_indices_filtered(
    m::Model;
    node=anything,
    ...
    filter(f, expression_represented_node_state_indices(m))
end
