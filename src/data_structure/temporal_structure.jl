...
Generate an `Array` of all valid `(node, time_slice)` `NamedTuples` with keyword arguments that allow filtering.
"""
...
function node_dynamic_time_indices(
    m::Model; node=anything, temporal_block=anything, t_before=anything, t_after=anything
)
    unique(
