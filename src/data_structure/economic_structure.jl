...
function generate_unit_annuity!(m::Model)
    ...
    for u in unit()
        ...
        # @show timeseries_ind
        unit.parameter_values[u][:annuity] = parameter_value(TimeSeries(timeseries_ind,timeseries_val,false,false))
    end
...
function generate_salvage_fraction!(m::Model)
    ...
    for u in unit()
        ...
        # @show timeseries_ind
        unit.parameter_values[u][:salvage_fraction] = parameter_value(TimeSeries(timeseries_ind,timeseries_val,false,false))
    end
...
function generate_tech_discount_factor!(m::Model)
    ...
    for u in indices(discount_rate_technology_specific) #@TIm what would be the default value (for unit w/o)? 0?
        ...
        unit.parameter_values[u][:tech_discount_factor] = parameter_value(val)
    end
