...
function process_master_problem_solution(m_bm)
    function _save_mp_values(
        ...
        for obj in indices(investment_parameter)
            ...
            inds_vals = [
                (start(ind.t), m_bm.ext[:spineopt].values[investment_variable_name][ind])
                for ind in variable_indices(m_bm; Dict(obj_cls.name => obj)...) if end_(ind.t) <= end_(current_window(m_bm))
            ]
...
function process_subproblem_solution(m, m_bm)
    save_sp_marginal_values(m)
    save_sp_objective_value_bi(m, m_bm)
...
function save_sp_objective_value_bi(m, m_bm)
    ...
    total_mp_investment_costs = reduce(+, values(m_bm.ext[:spineopt].values[:unit_investment_costs]); init=0)
    total_mp_investment_costs += reduce(+, values(m_bm.ext[:spineopt].values[:connection_investment_costs]); init=0)

    obj_ub = m_bm.ext[:spineopt].objective_upper_bound = total_sp_obj_val + total_mp_investment_costs
    obj_lb = m_bm.ext[:spineopt].objective_lower_bound = reduce(+, values(m_bm.ext[:spineopt].values[:mp_objective_lowerbound]); init=0)
    m_bm.ext[:spineopt].benders_gap = (2 * (obj_ub - obj_lb)) / (obj_ub + obj_lb)
end
