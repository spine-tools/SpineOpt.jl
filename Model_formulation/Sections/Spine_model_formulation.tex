...
%\textbf{N}: Network Matrix
%\textbf{L}: Loss-transfer Matrix
%\textbf{$v^{Trans}$}: stream of transfers
...
\end{itemize}
% \begin{itemize}
% 	\item only one commodity at a time ({\color{red} this constraint could be applied for more than two flow connections})
% \end{itemize}
% \begin{align} \label{eq:transconstraints1}
...
\end{itemize}
	\begin{align} \label{eq:transconstraints1}
		&\sum_{n_i,n_j \in }\vTranCapa \leq \pMaxTransCapa[con,sum]\\  %+ \pMaxTransCapa[con,n_j,n_i,t]\\
		% &\vTranCapa[con,n_k,n_l,t] + \vTranCapa[con,n_l,n_k,t] \leq \pMaxTransCapa[con,n_k,n_l,t]  + \pMaxTransCapa[con,n_l,n_k,t]
	\end{align}
\subsubsection{User defined dependencies (shares of certain flows to be constrained)}

\begin{align} \label{eq:transconstraints1}
	&\vTranCapa \leq p^{Share}_{con,n_i,n_j} \pMaxTransCapa[con,sum]\\  %+ \pMaxTransCapa[con,n_j,n_i,t]\\
	% &\vTranCapa[con,n_k,n_l,t] + \vTranCapa[con,n_l,n_k,t] \leq \pMaxTransCapa[con,n_k,n_l,t]  + \pMaxTransCapa[con,n_l,n_k,t]
\end{align}
...
%\begin{align}
%	& v_{n_i} = \mathrm{B}^{bus} \theta\\
%	& v_{Nodal} = \pDemand + \vFlow [\node_i,u,in/out,t] \\
%	& \vTrans = \sum_n_{i} \mathrm{PTDF}_{(n_i,n_j),n_i} \cdot v_{n_i}
%	& FNl,t = ISFl,n PNn,t ∀ l, ∀ t
%&P T DF N l,n1−n2 = ISFl,n1 − ISFl,n
%&XnPNn,t = 0 ∀ t \\
%& FZ = PTDFZ PZ + FZ\\
%& FZk,t =XzP T DF Zk,z PZz,t + FZ,0z ∀k, ∀ t\\
%& improved way:\\
%& ISKY
%& n,z,t =
%& P
...
%\end{align}
%\begin{itemize}
%	\item $\mathrm{B}^{bus}$ N-by-N bus admittance matrix
%	\item $\theta$ N-by-1 vector with nodal voltage angles
%	\item ISFL: L-by-N matrix with injection shift factors
%	\item necessary define a reference node
...
\end{itemize}
%\begin{itemize}
%\item {\color{red} issue with current formulation:\\ the formultion doesn't allow simultanously
%constraining some commodity flows to the same flow direction and
% at the same time excluding other commodities, as this would result in the multiplication of two decision %variables. }
% \item {\color{red} REMEMBER THAT capa is a positive (!) variable, is this important? revise equations}%
  %\end{itemize}
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \subsection{Geographical constraints}
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
