# [How to manage Output Data](@id how-to-manage-output)
...
To report the dual of a constraint, one can add an output item with the corresponding constraint name (e.g. [constraint\_nodal_balance](@ref constraint_nodal_balance)) and add that to a report. This will cause the corresponding constraint's marginal value to be reported in the output DB. When adding a constraint name as an output we need to preface the actual constraint name with `constraint_` to avoid ambiguity with variable names. So to report the marginal value of [constraint\_units\__available](@ref constraint_units_available) we add an output object with that name.
...
## Output Writing Summary
 - We need an output object in our input datastore for each variable or marginal value we want included in a report
 ...
 - Model outputs are tagged with alternatives that are unique to the model run and scenario that generated them
