...
- You can change the solvers in your input datastore using the [solver\_lp](@ref) and [solver\_mip](@ref) parameter values of the [model](@ref) object.
- You can specify solver options via the [solver\_lp\_options](@ref) and [solver\_mip\_options](@ref) parameters respectively. These are map parameters where the first key is the solver name exactly as the [solver\_mip](@ref) or [solver\_lp](@ref) name, the second key is the solver option name and the value is the option value.
- You can get a head start by copying the default map values for [solver\_lp\_options](@ref) and [solver\_mip\_options](@ref). You can access the default values by clicking on the 'Object parameter definition' tab.
