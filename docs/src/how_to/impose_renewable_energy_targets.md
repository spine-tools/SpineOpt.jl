...
Three nodes are required to represent this system: an `electricity` node, a `carbon_cap_1` node (with `storage_activate=true` and `storage_state_max=100`), and a `carbon_cap_2` node (with `storage_activate=true` and `storage_state_max=50`).
