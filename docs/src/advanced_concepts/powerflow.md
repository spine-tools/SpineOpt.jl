...
4. **Controlling problem size**
   - The lines to be monitored are specified by setting the [monitoring\_active](@ref) property for each connection for which a flow constraint is to be generated
   - The contingencies to be considered are specified by setting the [contingency\_active](@ref) property for the appropriate connections. For N contingencies and M monitored lines, N x M constraints will be generated.
