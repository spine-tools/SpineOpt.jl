...
* [duration\_unit](@ref) (optional): "minute or hour"
 This parameter gives the unit of duration that is used in the model calculations. The default value for this parameter is 'minute'.
...
#### The [temporal\_block](@ref) object
...
#### The [model\_\_default\_temporal\_block](@ref) relationship
...
#### The [node\_\_temporal\_block](@ref) relationship
...
#### The [units\_on\_\_temporal\_block](@ref) relationship
This relationship links the [units\_on](@ref var_units_on)(@ref var_units_on) variable of a unit to a temporal block and will therefore govern the time-resolution of the unit's online/offline status.

#### The [unit\_\_investment\_temporal\_block](@ref) relationship
...
#### The [model\_\_default\_investment\_temporal\_block](@ref) relationship
...
A model with a single [temporal\_block](@ref) can also be optimized in a rolling horizon framework. In this case, the [roll\_forward](@ref) parameter has to be defined in the [model](@ref) object. The [roll\_forward](@ref) parameter will then determine how much the optimization moves forward with every step, while the size of the temporal block will determine how large a time frame is optimized in each step. To see this more clearly, let's take a look at an example.
...
### Advanced usage: multiple [temporal\_block](@ref) objects
