...
then it is assumed to have only one child: the [model](@ref) itself.
...
the [node\_state](@ref var_node_state) [output](@ref) and the [node](@ref).
...
1. Create a [stage\_\_output\_\_node](@ref) between your [stage](@ref), the [node\_state](@ref var_node_state) [output](@ref) and your storage [node](@ref).
