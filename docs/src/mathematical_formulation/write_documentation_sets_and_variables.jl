...
function write_documentation_sets_variables()
    ...
    for i in 1:size(sets, 1)
        if !isempty(findall(x -> x == sets.indices[i], variables.indices))
            indexed_variables =
                join(variables.variable_name[findall(x -> x == sets.indices[i], variables.indices)], ",")
            set_string = string(
                set_string,
                "| ``$(sets.index[i]) \\in $(sets.indices[i])`` | Indices of the variable(s) $(indexed_variables) | \n",
            )
        else
            set_string =
                string(set_string, "| ``$(sets.index[i]) \\in $(sets.indices[i])`` | $(sets.Description[i]) | \n")
        end
    end
