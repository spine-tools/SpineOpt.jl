The [node](@ref) is perhaps the most important class out of the [Systemic object classes](@ref),
...
The [storage\_active](@ref) parameter controls whether a [node](@ref) has a [node\_state](@ref var_node_state) variable,
which essentially represents the commodity content of the [node](@ref).
The [storage\_state\_coefficient](@ref) parameter tells how the [node\_state](@ref var_node_state) variable relates to all the commodity flows.
