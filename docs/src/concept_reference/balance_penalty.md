`balance_penalty` triggers the creation of node slack variables, `node_slack_pos` and `node_slack_neg`.
...
with a coefficient equal to `balance_penalty`. If `balance_penalty` = 0 the slack variables are created and violations are
