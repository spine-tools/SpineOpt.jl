...
To enable node-based lossless DC powerflow, each node will be associated with a [node\_voltage\_angle](@ref var_node_voltage_angle) variable. 
...
[node\_pressure](@ref var_node_pressure) variable. The pressure at a certain node can also be constrainted through the parameters 
