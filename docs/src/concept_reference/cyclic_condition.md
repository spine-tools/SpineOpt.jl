...
for a [node\_\_temporal\_block](@ref) relationship, and the [storage\_activate](@ref) parameter of the corresponding [node](@ref) is set to [true](@ref boolean_value_list), the [constraint\_cyclic\_node\_state](@ref) will be triggered.
