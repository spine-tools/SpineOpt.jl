...
used by the [units\_on](@ref var_units_on) variable.
Essentially, this relationship permits defining a different [stochastic\_structure](@ref) for the online decisions
regarding the [units\_on](@ref var_units_on) variable,
than what is used for the production [unit\_flow](@ref var_unit_flow) variables.
