The [storage\_state\_coefficient](@ref) parameter acts as a coefficient for the [node\_state](@ref var_node_state) variable
in the [node injection constraint](@ref constraint_node_injection).
Essentially, it tells how the [node\_state](@ref var_node_state) variable should be treated in relation to the commodity flows
...
the [node](@ref) must first have a [node\_state](@ref var_node_state) variable to begin with,
defined using the [storage\_active](@ref) parameter.
