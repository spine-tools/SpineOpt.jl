...
For each [unit\_\_to\_node](@ref) or [node\_\_to\_unit](@ref), a [unit\_flow](@ref var_unit_flow) variable is automatically
...
on a group of nodes will result in an upper bound on the sum of all individual [unit\_flow](@ref var_unit_flow)s.
