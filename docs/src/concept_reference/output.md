...
Typically, e.g. the [unit\_flow](@ref var_unit_flow) variables are desired as output from most [model](@ref)s,
so creating an [output](@ref) object called [unit\_flow](@ref var_unit_flow) allows one to designate it as something to be written in the
desired [report](@ref).
