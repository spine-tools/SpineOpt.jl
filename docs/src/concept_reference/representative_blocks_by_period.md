...
In other words, the model always creates [node\_state](@ref var_node_state) variables and investment variables for all
time periods, regardless of whether [representative\_blocks\_by\_period](@ref) is specified for any
[temporal\_block](@ref).

To use representative periods in your model, do the following:

1. Define one [temporal\_block](@ref) for the 'normal' periods as you would do if you weren't
...
3. Specify [representative\_blocks\_by\_period](@ref) for the 'normal' [temporal\_block](@ref) as a *map*,
   from consecutive date-time values to the name of a representative [temporal\_block](@ref).
4. Associate all the above [temporal\_block](@ref) objects to elements in your model
...
See also [How to set up representative days for investment problems](@ref usage_rep_period_seasonal_storage).