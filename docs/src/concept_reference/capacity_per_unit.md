...
or [node\_\_to\_unit](@ref) relationship. By defining the parameter, the [unit\_flow](@ref var_unit_flow) variables
...
Note that if the [capacity\_per\_unit](@ref) parameter is defined on a node group, the sum of all [unit\_flow](@ref var_unit_flow)s
