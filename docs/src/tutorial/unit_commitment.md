...
- Change the *Parameter type* to *Time series fixed resolution*, [resolution](@ref) to *1h*, and the demand values to the time series as in the image below. You can copy and paste the values from the file: [uc\_electricity\_node\_demand.csv](figs_unit_commitment/uc_electricity_node_demand.csv)
- Finish by pressing *OK* in the *Edit value* menu. In the *Object parameter* table you will see that the value of the [demand](@ref) has changed to *Time series*.
...
- Locate the *Object parameter* table (typically at the top-center).
- In the *Object parameter* table, identify the [resolution](@ref) parameter which should have a *1D* value from the Simple System first run.
- Right-click on the value cell and then select *edit* from the context menu. The *Edit value* dialog will pop up.
...
- Enter *report1* under [report](@ref), and [units\_on](@ref var_units_on) under [output](@ref). Repeat the same procedure for the following outputs as seen in the image below; then press *OK*.
...
- Remember to select the latest run in the *Alternative tree*. Expand the [output](@ref) element if not expanded.
...
- In the Spine DB editor, locate the *Relationship tree* (typically at the bottom-left). Expand the *root* element if not expanded.
- In *Relationship tree*, expand the [unit\_\_to\_node](@ref) class and select *power\_plant\_b | electricity\_node*.
- In the *Relationship parameter* table (typically at the bottom-center), select the [minimum\_operating\_point](@ref) parameter and the *Base* alternative, and enter the value *0.1* as seen in the image below. This will set the minimum operating point of *power\_plant\_b* when producing electricity.
...
- In the *Object parameter* table (typically at the top-center), select the following parameter as seen in the image below:
  - [online\_variable\_type](@ref) parameter and the *Base* alternative, and select the value *binary*. This will define that the unit commitment variables will be binary. SpineOpt identifies this situation from the input data and internally changes the model from LP to MIP.
  - [shut\_down\_cost](@ref) parameter and the *Base* alternative, and enter the value *7*. This will establish that there's a cost of '7' EUR per shutdown.
  - [start\_up\_cost](@ref) parameter and the *Base* alternative, and enter the value *5*. This will establish that there's a cost of '5' EUR per startup.
  - [units\_on\_cost](@ref) parameter and the *Base* alternative, and enter the value *3*. This will establish that there's a cost of '3' EUR per units on (e.g., idling cost).
  - [online\_count\_initial](@ref) parameter and the *Base* alternative, and enter the value *0*. This will establish that there are no units 'on' before the first time step.
...
- Locate the *Object tree* (typically at the top-left). Expand the [root] element if not expanded.
- Expand the [unit] class, and select the *power\_plant\_b* from the expanded tree.
- In the *Object parameter* table (typically at the top-center), select the [min\_up\_time](@ref) parameter, the *Base* alternative, and then right-click on the value and select the *Edit* option in the context menu, as shown in the image below.
...
- Let's take a look again to the [units\_on](@ref var_units_on) and [units\_started\_up](@ref var_units_started_up) in the image below.
...
- Locate the *Object tree* (typically at the top-left). Expand the [root] element if not expanded.
- Expand the [unit] class, and select the *power\_plant\_b* from the expanded tree.
- In the *Object parameter* table (typically at the top-center), select the [min\_down\_time](@ref) parameter, the *Base* alternative, and then right-click on the value and select the *Edit* option in the context menu, as shown in the image below.
