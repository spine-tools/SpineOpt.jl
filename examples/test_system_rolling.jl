...
# run_spinemodel(db_url,rolling = :rolling_DA)
...
using_spinedb(db_url, SpineModel)
for (window, block_time_slices) in enumerate(SpineModel.window_block_time_slices(:rolling_DA))
    @show window
